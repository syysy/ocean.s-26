		...
		while True:
			...
			if line:
				...
				if magnet_count < self.last_magnet_count:
					# Si on était à 3 et qu'on avait un timer, reset
					if self.last_magnet_count == 3 and self.max_reached_time is not None:
				...
				# Gestion du timer pour 3 magnets
				if magnet_count == 3:
					...
					# Reset le timer si on n'est pas à 3
					self.max_reached_time = None
